...
// ✅ UPDATED: Added getAllUserDeviceTokens method
...
export const NotificationsModel = {
  /* ============================================
     ...
     (createNotification, createBroadcastNotification, etc.)
  ============================================ */
  ...
  `;
    ...
    // ✅ For non‑admin users: exclude SOS alerts, include broadcasts
    if (!["admin", "superadmin", "police"].includes(userRole)) {
      query += ` AND n.type NOT IN ('sos_alert')`;
    }
  ...
  async countUserNotifications({ userId, userRole, category, isRead, type }) {
    let query = `
  ...
  `;
    ...
    // ✅ For non‑admin users: exclude SOS alerts
    if (!["admin", "superadmin", "police"].includes(userRole)) {
      query += ` AND n.type NOT IN ('sos_alert')`;
    }
  ...
  /* ============================================
     ✅ DEVICE TOKENS - Your existing methods + one new one
  ============================================ */
  ...
  async getUserDeviceTokens(userId) {
    const query = `
      SELECT token, device_type, device_info, created_at, last_used_at
      FROM device_tokens
      WHERE user_id = $1
      AND is_active = TRUE;
    `;
  ...
  /* ============================================
   DEVICE TOKENS - GET ALL ACTIVE USER TOKENS (for broadcast)
============================================ */
  async getAllUserDeviceTokens() {
    const query = `
    SELECT dt.token, dt.device_type, u.id as user_id
  ...
  // // ✅ NEW: Get all active user tokens (for broadcast)
  // async getAllUserDeviceTokens() {
  //   const query = `
  //     SELECT dt.token, dt.device_type, u.id as user_id
  //     FROM device_tokens dt
